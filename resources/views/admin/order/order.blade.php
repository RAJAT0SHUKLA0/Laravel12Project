                        ...
                        <div class="card-body pt-0">
                            <div>
                                ...
                                @if(count($statusLabels))
                                    <ul class="nav">
                                          @foreach($statusLabels as $label => $value)
                                            <li class="nav-item">
                                                <form action="{{ route('order') }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="order_status" value="{{ $value }}">
                                                    <button type="submit"
                                                            class="nav-link px-3 py-2 {{ (request('order_status') === null && $label === 'All Orders') || request('order_status') == $value ? 'active' : '' }}"
                                                            style="border: none; background: none; cursor: pointer;">
                                                        <i class="{{ $iconClass[$loop->index] ?? '' }} me-1 align-bottom"></i>{{ $label }}
                                                    </button>
                                                </form>
                                            </li>
                                        @endforeach
                                                ...
                                                <td>
                                                    <ul class="list-inline hstack gap-2 mb-0">
                                                        <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" aria-label="View" data-bs-original-title="View">
                                                            <a href="@encryptedRoute('orderDetails',$order->id)" class="text-primary d-inline-block">
